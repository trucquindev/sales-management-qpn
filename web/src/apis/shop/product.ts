...
export const postShoppingCard = async (customerId: string, productId: string) => {
  const response = await authorizedAxiosInstance.post('/api/ShoppingCard', { customerId, productId });
  return response.data;
}
export const getShoppingCardCustomer = async (customerId: string) => {
  ...
  export const updateShoppingCard = async (id: string, quantity: number, productID: string, customerId: string) => {
    await authorizedAxiosInstance.put('/api/ShoppingCard', { id, quantity, productID, customerId });
  }